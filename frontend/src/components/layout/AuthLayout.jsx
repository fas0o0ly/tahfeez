    ...
    <div className="hidden lg:flex lg:w-[45%] relative flex-col justify-between
      ...
      >
        <Link to="/" className="flex items-center gap-3 mb-3 group w-fit">
          <img src={logoImg} alt="Tahfeez" className="w-10 h-10 object-contain transition-transform group-hover:scale-105" />
          <span className="text-white font-display text-xl font-semibold tracking-wide group-hover:text-gold-300 transition-colors">
            Tahfeez
          </span>
        </Link>
      ...
      >
        <p
          ...
          style={{ fontFamily: 'Amiri, serif', fontSize: '28px', direction: 'rtl', textShadow: '0 2px 20px rgba(212,175,55,0.3)' }}
        >
قال الله تعالى: ﴿وَلَقَدْ يَسَّرْنَا الْقُرْآنَ لِلذِّكْرِ فَهَلْ مِن مُّدَّكِرٍ﴾ [القمر: ١٧]
        ...
        <p className="text-forest-500 text-xs mt-2 tracking-widest">— AL-QAMAR 54:17</p>
      </motion.div>
      ...
      >
        {[
          { icon: '📖', key: 'auth.layout.feature1' },
          { icon: '🎙️', key: 'auth.layout.feature2' },
          { icon: '📊', key: 'auth.layout.feature3' },
        ].map((item) => (
          <div key={item.key} className="flex items-center gap-3">
            ...
            <p className="text-forest-300 text-xs leading-relaxed">{t(item.key)}</p>
          </div>
        ))}
      </motion.div>
    </div>
    ...
    <div className="min-h-screen flex" style={{ backgroundColor: '#f9f7f2' }}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12 overflow-y-auto">
        <div className="lg:hidden mb-8 text-center">
          ...
          <p className="text-forest-500 text-xs tracking-widest">{t('auth.layout.tagline')}</p>
        </div>
